...
public class BaiduMusicService {  
    ...
    public static Music searchMusic(String musicTitle, String musicAuthor) {  
        ...
        // 对音乐名称、作者进URL编码  
        requestUrl = requestUrl.replace("{TITLE}", EncodeUtil.encodeUTF8(musicTitle));  
        requestUrl = requestUrl.replace("{AUTHOR}", EncodeUtil.encodeUTF8(musicAuthor));  
        // 处理名称、作者中间的空格  
        requestUrl = requestUrl.replaceAll("\\+", "%20");  
  
        // 查询并获取返回结果  
        InputStream inputStream = HttpRequestUtil.gethttpRequestIS(requestUrl);  
        // 从返回结果中解析出Music  
